...
class TelaVinculacao(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        card = ctk.CTkFrame(
            self, fg_color=CARD, border_color=BORDER, border_width=1, corner_radius=16
        )
        ...
        if logo:
            ...
            ctk.CTkLabel(card, text="👻", font=("Segoe UI", 48)).grid(
                row=0, column=0, pady=(32, 0)
            )

        ctk.CTkLabel(
            card, text="PhantomFix", font=("Segoe UI", 28, "bold"), text_color=TEXT_MAIN
        ).grid(row=1, column=0, pady=(8, 0))
        ctk.CTkLabel(
            card,
            text="Cole seu token para vincular esta máquina à sua conta.",
            font=("Segoe UI", 13),
            text_color=TEXT_DIM,
            wraplength=340,
        ).grid(row=2, column=0, pady=(6, 24))
    ...
    def _vincular(self):
        ...
        if not token:
            self.lbl_status.configure(
                text="Cole o token antes de continuar.", text_color=DANGER
            )
            return
    ...
    def _buscar_nome(self, token: str) -> str:
        try:
            resp = requests.get(ME_URL, params={"token": token}, timeout=5)
            if resp.status_code == 200:
                return resp.json().get("nome", "") or ""
        except Exception:
            pass
        return ""
...
class TelaAnalise(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(
            header,
            text="ANÁLISE DE VULNERABILIDADES",
        ...
        ctk.CTkLabel(
            badge,
            text=f"✓  Conta vinculada  ·  {self.nome}",
        ...
        self.lbl_repo = ctk.CTkLabel(
            card_repo,
            text="Selecione a pasta do seu projeto para iniciar a análise.",
            font=("Segoe UI", 12),
            text_color=TEXT_DIM,
        )
        ...
        ctk.CTkLabel(
            card_url,
            text="Informe a URL em produção para o teste dinâmico.",
            font=("Segoe UI", 12),
            text_color=TEXT_DIM,
        ...
        self.lbl_status = ctk.CTkLabel(
            si,
            text="Selecione um repositório para iniciar.",
            font=("Segoe UI", 12),
            text_color=TEXT_DIM,
        )
    ...
    def _confirmar_desvincular(self):
        ok = messagebox.askyesno(
            "Trocar conta",
            "Deseja desvincular esta máquina e trocar de conta?\nSeu token local será apagado.",
        )
        if ok:
            apagar_token()
            self.on_desvincular()
    ...
    def _zipar(self, pasta, destino):
        ...
        with zipfile.ZipFile(destino, "w", zipfile.ZIP_DEFLATED) as zf:
            for arq in pasta.rglob("*"):
                if arq.is_file() and not any(
                    p in ignorar for p in arq.relative_to(pasta).parts
                ):
...
class PhantomFixApp(ctk.CTk):
    def __init__(self):
        ...
        if token:
            self._ir_vinculacao_com_token(token)
        else:
            self._ir_vinculacao()
